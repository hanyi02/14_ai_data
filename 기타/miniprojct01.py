...
def N_back():
    ...
    print(f"\n점수: 맞은 개수 × {SCORE_PER_HIT}점 (최대 {(18 - N_BACK) * SCORE_PER_HIT}점)")
    input("\n엔터 누르세요")
    ...
    for turn in range(1, 19):  # 1턴부터 18턴까지
        symbol= random.choice(SYMBOLS)  #기호 무작위 선택
        sequence.append(symbol)  # 기록에 추가

        # 기호 표시
        clear()
        print(f"1. 특수기호 2-back 게임   턴: {turn}/{18}\n")
        print("\n" * 4)
        print(" " * 30 + symbol)
...
def result():
    clear()
    print("게임 종료\n")
    print(f"1. 특수기호 2-back: {score1}점 / {(18 - N_BACK) * SCORE_PER_HIT}점")
